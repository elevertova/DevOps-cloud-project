...
#   for_each = {
#     for dvo in aws_acm_certificate.frhn-dev_cert.domain_validation_options : dvo.domain_name => {
...
# resource "aws_acm_certificate_validation" "frhn-dev_cert_validation" {
#   certificate_arn         = aws_acm_certificate.frhn-dev_cert.arn
...
resource "aws_instance" "frhn-dev" {
  ...
  tags = { Name = "frhn-dev-app-server" }
}

# Load Balancer for HTTP and HTTPS traffic
resource "aws_lb" "frhn-dev_lb" {
  name               = "frhn-dev-app-lb"
  internal           = false
  load_balancer_type = "application"
  security_groups    = [aws_security_group.vpc_web_sg.id]
  subnets            = [aws_subnet.public_subnet_1.id, aws_subnet.public_subnet_2.id]

  tags = { Name = "frhn-dev-app-lb" }
}

# Target Group pointing to the EC2 instance
resource "aws_lb_target_group" "frhn-dev_tg" {
  name        = "frhn-dev-app-tg"
  port        = 80
  protocol    = "HTTP"
  vpc_id      = aws_vpc.frhn-dev.id
  ...
  tags = { Name = "frhn-dev-app-tg" }
}

# Register the EC2 instance with the Target Group
resource "aws_lb_target_group_attachment" "frhn-dev_tg_attachment" {
  target_group_arn = aws_lb_target_group.frhn-dev_tg.arn
  target_id        = aws_instance.frhn-dev.id
  port             = 80
}

# HTTP Listener to redirect traffic to HTTPS
resource "aws_lb_listener" "frhn-dev_http_listener" {
  load_balancer_arn = aws_lb.frhn-dev_lb.arn
  port              = 80
  protocol          = "HTTP"
...
# resource "aws_lb_listener" "frhn-dev_https_listener" {
#   load_balancer_arn = aws_lb.frhn-dev_lb.arn
#   port              = 443
#   protocol          = "HTTPS"
...
resource "aws_route53_record" "frhn-dev_record" {
  zone_id = aws_route53_zone.mycloudlab_zone.zone_id
  name    = "mycloudlab.space"
  type    = "A"
  alias {
    name                   = aws_lb.frhn-dev_lb.dns_name
    zone_id                = aws_lb.frhn-dev_lb.zone_id
