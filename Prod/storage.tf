...
# S3 Bucket for application storage with private access
resource "aws_s3_bucket" "frhn_prod_app_bucket" {
  bucket = "frhn-prod-app-logs-${random_id.bucket_suffix.hex}"

  tags = {
    Name = "frhn-uay-app-logs"
  }
}

# Enforce ownership controls on the bucket
resource "aws_s3_bucket_ownership_controls" "frhn_prod_app_bucket_ownership_controls" {
  bucket = aws_s3_bucket.frhn_prod_app_bucket.id
    ...
    Statement = [
      {
        ...
        Resource = [
          "${aws_s3_bucket.frhn_prod_app_bucket.arn}",
          "${aws_s3_bucket.frhn_prod_app_bucket.arn}/*"
        ]
      }
    ]
...
resource "aws_s3_bucket_policy" "frhn_prod_app_bucket_policy" {
  bucket = aws_s3_bucket.frhn_prod_app_bucket.id
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Allow CloudTrail to write logs to this bucket
      {
        ...
        Action   = "s3:PutObject",
        Resource = "${aws_s3_bucket.frhn_prod_app_bucket.arn}/AWSLogs/${data.aws_caller_identity.current.account_id}/*",
        Condition = {
          StringEquals = {
...
resource "aws_ebs_volume" "frhn-prod-app_web_ebs" {
  availability_zone = "us-west-2a" # Corrected spelling
  size              = 10           # Size in GB
  tags = {
    Name = "frhn-prod-app-web-ebs"
  }
}
